...
pub fn a_star(
    mut nodes: HashMap<Vec2Int, Tile>,
    start: Vec2Int,
    mut end: Vec2Int,
) -> Option<Vec<Vec2Int>> {
    ...
    while !open_nodes.is_empty() {
        ...
        let current_node_pos = {
            ...
            for (n_index, node_pos) in open_nodes.iter().enumerate() {
                let (current_f, current_g) = (
                    nodes.get(&current_position)?.f,
                    nodes.get(&current_position)?.g,
                );
...
fn get_neighbours_valid_positions(
    nodes: &HashMap<Vec2Int, Tile>,
    base_position: Vec2Int,
) -> Vec<Vec2Int> {
